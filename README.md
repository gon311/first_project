...
| 이름 | 역할 | 담당 |
|---|---|---|
| 이다겸 | 팀장 |  메인화면(구직자/기업), AI자소서첨삭 및 맞춤법, 문장 다듬기 구현, Git 관리 |
| 조수연 | 서기 |  관리자페이지 메인, 게시판 (채용공고, 커뮤니티, qna, faq, 공지사항), <br>배너 관리, 데이터 관리   |
| 이재우 | 팀원 |  마이페이지(구직자/기업), 커뮤니티(게시판, 댓글) |
| 이호철 | 팀원 | 이력서 작성 및 수정, 삭제 |
| 임재호 | 팀원 |  공고등록 및 수정(기업), 지원자 관리(기업), 공고리스트(구직자), <br>공고 상세페이지(구직자), 공고 지원(구직자) |
| 최선린 | 팀원 |  관리자 페이지(회원 관리, 제출된 공고 관리, 결제 관리), 유료 서비스 결제  |
| 최승우 | 팀원 | 로그인 및 회원가입, 아이디/비밀번호 찾기, 공지사항 |
