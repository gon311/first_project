...
<body>
	...
	<main class="container-fluid px-5 mt-4">
		...
		<div class="tab-content mt-3" id="memberTabContent">
			...
			<!-- 페이징 -->
			<c:if test="${not empty param.keyword or not empty param.startDate or not empty param.endDate or not empty param.userType or not empty param.payStatus}">
				<c:set var="searchParams"
				value="keyword=${param.keyword}&startDate=${param.startDate}&endDate=${param.endDate}&userType=${param.userType}&payStatus=${param.payStatus}" />
			</c:if>
			...
			<nav>
				<ul class="pagination justify-content-center">
					<li class="page-item <c:if test="${pageInfo.pageNum eq 1}">disabled</c:if>">
						<a class="page-link" href="<c:url value="/admin/payments?pageNum=${pageInfo.pageNum - 1}&${searchParams}&activeTab=${activeTab}" />">&laquo;</a>
					</li>
					...
					<c:forEach var="i" begin="${pageInfo.startPage}" end="${pageInfo.endPage}">
						<li class="page-item <c:if test="${i eq pageInfo.pageNum}">active</c:if>">
							<c:choose>
								...
								<c:otherwise>
									<a class="page-link" href="<c:url value="/admin/payments?pageNum=${i}&${searchParams}&activeTab=${activeTab}" />">${i}</a>
								</c:otherwise>
							</c:choose>
						</li>
					</c:forEach>
					...
					<li class="page-item <c:if test="${pageInfo.pageNum eq pageInfo.maxPage}">disabled</c:if>">
						<a class="page-link" href="<c:url value="/admin/payments?pageNum=${pageInfo.pageNum + 1}&${searchParams}&activeTab=${activeTab}" />">&raquo;</a>
					</li>
				</ul>
			</nav>
		</c:if>
