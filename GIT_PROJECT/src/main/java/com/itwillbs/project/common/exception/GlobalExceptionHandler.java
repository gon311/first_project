...
public class GlobalExceptionHandler {
	...
	public String handleLoginRequired(LoginRequiredException e, Model model) {
		// Model 객체에 예외 처리 과정에서 필요한 메세지 및 이동 방식과 이동할 주소 등을 저장
		...
		// ----------------------------------------------------------------
		// 공통 에러 메세지 출력에 필요한 /common/alert.jsp 페이지로 포워딩(컨트롤러 사용법과 동일함)
		return "/common/alert";
	}
	...
	public String handleEtc(Exception e) {
		e.printStackTrace();
		// 나머지 모든 에러 메세지 출력에 필요한 /common/error.jsp 페이지로 포워딩(컨트롤러 사용법과 동일함)
		return "/common/error";
	}
