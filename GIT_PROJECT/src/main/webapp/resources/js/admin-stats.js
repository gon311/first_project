...
const STAT_CONFIG = {
    user: {
        ...
        apiUrl: "/admin/api/user-stats"
    },
    com: {
        titles: ["기업별 공고 등록수", "모집 분야별 비중", "고용 형태 분포"],
        apiUrl: "/admin/api/com-stats"
    },
    userPay: {
        titles: ["구직자 상품 판매량", "결제 수단 비중", "최근 매출 추이"],
        apiUrl: "/admin/api/user-pay-stats"
    },
    comPay: {
        titles: ["매출 상위 기업(TOP 5)", "기업 상품 판매 비중", "결제 수단"],
        apiUrl: "/admin/api/com-pay-stats"
    }
};
...
function renderChartsByType(type, data) {
    if (type === 'user') {
        ...
        createChart('chart-canvas-3', 'doughnut', data.methods);
    }
}
